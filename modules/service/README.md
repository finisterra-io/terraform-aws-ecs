...
| [aws_service_discovery_service.selected](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/service_discovery_service) | data source |
...
| <a name="input_autoscaling_max_capacity"></a> [autoscaling\_max\_capacity](#input\_autoscaling\_max\_capacity) | Maximum number of tasks to run in your service | `number` | `null` | no |
| <a name="input_autoscaling_min_capacity"></a> [autoscaling\_min\_capacity](#input\_autoscaling\_min\_capacity) | Minimum number of tasks to run in your service | `number` | `null` | no |
...
| <a name="input_autoscaling_tags"></a> [autoscaling\_tags](#input\_autoscaling\_tags) | A map of additional tags to add to the autoscaling group created | `map(string)` | `{}` | no |
...
| <a name="input_cloudwatch_log_group_name"></a> [cloudwatch\_log\_group\_name](#input\_cloudwatch\_log\_group\_name) | Name to use on CloudWatch log group created | `string` | `null` | no |
...
| <a name="input_cpu"></a> [cpu](#input\_cpu) | Number of cpu units used by the task. If the `requires_compatibilities` is `FARGATE` this field is required | `number` | `null` | no |
| <a name="input_create"></a> [create](#input\_create) | Determines whether resources will be created (affects all resources) | `bool` | `true` | no |
| <a name="input_create_cloudwatch_log_group"></a> [create\_cloudwatch\_log\_group](#input\_create\_cloudwatch\_log\_group) | Determines whether to create a CloudWatch log group | `bool` | `false` | no |
| <a name="input_create_iam_policy"></a> [create\_iam\_policy](#input\_create\_iam\_policy) | Determines whether the ECS service IAM policy should be created | `bool` | `false` | no |
...
| <a name="input_execution_role_arn"></a> [execution\_role\_arn](#input\_execution\_role\_arn) | Existing IAM role ARN | `string` | `null` | no |
...
| <a name="input_iam_policy_description"></a> [iam\_policy\_description](#input\_iam\_policy\_description) | Description of the policy | `string` | `null` | no |
| <a name="input_iam_policy_document"></a> [iam\_policy\_document](#input\_iam\_policy\_document) | IAM policy document | `string` | `null` | no |
| <a name="input_iam_policy_name"></a> [iam\_policy\_name](#input\_iam\_policy\_name) | Name to use on IAM policy created | `string` | `null` | no |
| <a name="input_iam_policy_path"></a> [iam\_policy\_path](#input\_iam\_policy\_path) | IAM policy path | `string` | `null` | no |
| <a name="input_iam_role"></a> [iam\_role](#input\_iam\_role) | Existing IAM role | `string` | `null` | no |
...
| <a name="input_iam_role_inline_policies"></a> [iam\_role\_inline\_policies](#input\_iam\_role\_inline\_policies) | IAM policy document (same as policy\_documents but in JSON format) | `list(any)` | `[]` | no |
...
| <a name="input_ignore_task_definition_changes"></a> [ignore\_task\_definition\_changes](#input\_ignore\_task\_definition\_changes) | Whether changes to service `task_definition` changes should be ignored | `bool` | `true` | no |
...
| <a name="input_lb_listener_port"></a> [lb\_listener\_port](#input\_lb\_listener\_port) | Port to use on load balancer listener created | `number` | `80` | no |
| <a name="input_lb_listener_protocol"></a> [lb\_listener\_protocol](#input\_lb\_listener\_protocol) | Protocol to use on load balancer listener created | `string` | `"HTTP"` | no |
| <a name="input_lb_listener_tags"></a> [lb\_listener\_tags](#input\_lb\_listener\_tags) | A map of additional tags to add to the load balancer listener created | `map(string)` | `{}` | no |
| <a name="input_listener_rule_conditions"></a> [listener\_rule\_conditions](#input\_listener\_rule\_conditions) | List of conditions for the listener rule | `map(any)` | `{}` | no |
| <a name="input_listener_rule_priority"></a> [listener\_rule\_priority](#input\_listener\_rule\_priority) | Priority for the listener rule | `number` | `1` | no |
| <a name="input_listener_rule_tags"></a> [listener\_rule\_tags](#input\_listener\_rule\_tags) | A map of additional tags to add to the listener rule created | `map(string)` | `{}` | no |
| <a name="input_load_balancer"></a> [load\_balancer](#input\_load\_balancer) | Configuration block for load balancers | `list(any)` | `[]` | no |
| <a name="input_memory"></a> [memory](#input\_memory) | Amount (in MiB) of memory used by the task. If the `requires_compatibilities` is `FARGATE` this field is required | `number` | `null` | no |
...
| <a name="input_network_configuration"></a> [network\_configuration](#input\_network\_configuration) | Network configuration for the service | `list(any)` | `[]` | no |
| <a name="input_network_mode"></a> [network\_mode](#input\_network\_mode) | Docker networking mode to use for the containers in the task. Valid values are `none`, `bridge`, `awsvpc`, and `host` | `string` | `null` | no |
...
| <a name="input_subnet_names"></a> [subnet\_names](#input\_subnet\_names) | The names for the default subnets, uses a module label name if left empty | `list(string)` | `null` | no |
...
| <a name="input_task_exec_assume_role_policy"></a> [task\_exec\_assume\_role\_policy](#input\_task\_exec\_assume\_role\_policy) | IAM policy document defining the trust relationship for the task execution role | `string` | `null` | no |
...
| <a name="input_task_exec_iam_role_inline_policies"></a> [task\_exec\_iam\_role\_inline\_policies](#input\_task\_exec\_iam\_role\_inline\_policies) | IAM policy document (same as policy\_documents but in JSON format) | `list(any)` | `[]` | no |
...
| <a name="input_task_role_arn"></a> [task\_role\_arn](#input\_task\_role\_arn) | Existing IAM role ARN | `string` | `null` | no |
...
| <a name="input_tasks_assume_role_policy"></a> [tasks\_assume\_role\_policy](#input\_tasks\_assume\_role\_policy) | IAM policy document defining the trust relationship for the tasks role | `string` | `null` | no |
...
| <a name="input_tasks_iam_role_inline_policies"></a> [tasks\_iam\_role\_inline\_policies](#input\_tasks\_iam\_role\_inline\_policies) | IAM policy document (same as policy\_documents but in JSON format) | `list(any)` | `[]` | no |
...
| <a name="input_timeouts"></a> [timeouts](#input\_timeouts) | Create, update, and delete timeout configurations for the service | `map(string)` | `null` | no |
...
| <a name="input_vpc_id"></a> [vpc\_id](#input\_vpc\_id) | VPC ID to use on security group created | `string` | `null` | no |
